angular.module('CatalogoApp')
    .controller('AtencionController', function ($http, $sce, $location, $timeout) {
        self = this;
        ...
        checkAudio = function (id) {
            ...
            var promise = $http({
                method: 'GET',
                url: 'api/AtencionApi/GetRecording/' + id
            })
            .success(function (data, status, headers, config) {
        ...
        self.detailInit = function (tab) {
            ...
            checkAudio(atencion_id).then(function (promise) {
                console.log('AUDIO PROMISE', promise.data);
            })

            getAtencion(atencion_id).then(function (promise) {
                self.item = promise.data;
                self.item.url = self.getAudioURL(self.item);
                self.item.url2 = self.getAudioURL2(self.item);
        ...
        self.canPlayAudio = function () {
            console.log('can play');
            self.canPlayAudio = true;
        }

        self.download2 = function (id) {
            downloadURI(self.item.url2, self.item.LogUsr + '_' + self.item.NroTicket + '_' + self.item.fechaString + '.ogg');
            /*downloadAudio(id).then(function (promise) {
                self.response = promise.data;
                if (self.response.result) {
                    self.info = { state: true, message:'Audio exportado con éxito' }
        ...
        this.getAudioURL2 = function (item) {
        ...
        self.exportar = function (tab) {
            console.log('exportando');
            tab.templateUrl = 'filavirtual/atenciones/exportar/?ajax=1&_=' + Date.now;
        }
    });