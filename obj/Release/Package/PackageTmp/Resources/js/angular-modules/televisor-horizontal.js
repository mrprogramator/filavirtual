angular.module('TelevisorApp', [])
    .controller('TelevisorController', function ($scope, $http, $sce, $timeout, $location) {
        ...
        mainHub.on('length', function (length) {
            console.log('atencionesLength', length, $scope.atencionesLength);
            $scope.atencionesLength = length;
            $scope.exportadas = 0;
        });

        mainHub.on('exportada', function () {
            console.log('atención exportada', $scope.exportadas);
